...
- the default runtime now composes real server-backed clients for auth bootstrap, handle selection, feed, detail, and profile
- the live runtime now distinguishes dev/test local-identity auth from production bearer auth
...
- keep local server validation healthy for auth bootstrap, feed, detail, and profile
